# FEYZA ÖZLÜ e2580868
# I added the ICMPQ instruction. It has same functionality with the part-b in homework.
# It is comparing 2 registers and setting the condition codes accordingly but it is not updating the second register.
# I also added some additional comments at the end of lines that I made changes, which can be seen clearly.
...
word d_srcA = [
D_icode in { IRRMOVQ, IRMMOVQ, IOPQ, IPUSHQ, ICMPQ } : D_rA;						# since in ICMPQ instruction d_srcA comes from D_rA I added it here.
...
word d_srcB = [
	D_icode in { IOPQ, IRMMOVQ, IMRMOVQ, ICMPQ } : D_rB;							# since in ICMPQ instruction d_srcB comes from D_rB I added it here.
...
word aluA = [
    E_icode in { IRRMOVQ, IOPQ, ICMPQ } : E_valA; 									# in ICMPQ aluA = E_valA
...
E_icode in { IRMMOVQ, IMRMOVQ, IOPQ, ICALL,
		     IPUSHQ, IRET, IPOPQ, ICMPQ } : E_valB;								    # in ICMPQ aluB =E_ valB
...
word alufun = [
	E_icode == IOPQ : E_ifun;
	E_icode == ICMPQ : ALUSUB;											           # if instruction is ICMPQ then alufun should be ALUSUB to compare the registers
	1 : ALUADD;
];

## Should the condition codes be updated?
	bool set_cc = E_icode in { IOPQ, ICMPQ } &&								      # ICMPQ instruction should set the condition codes, but shouldn't update the second register
